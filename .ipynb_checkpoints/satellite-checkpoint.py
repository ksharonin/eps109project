...
class SatelliteDetection():
    ...
    def __set_up_master(self):
        ...
        if self._access_type == "api":
            assert self._title in SatelliteDetection.TITLE_SETS, "ERR SatelliteDetection: Invalid title provided"
            self.__set_api_url()
    ...
    def __set_api_url(self):
        ...
        if self._title in SatelliteDetection.OGC_URLS:
            self._api_url = SatelliteDetection.OGC_URLS[self._title]
    ...
    def __fetch_api_collection(self) -> dict:
        ...
        assert self._api_url is not None, "ERR SatelliteDetection: cannot fetch with a null API url"
    ...
    def __set_api_polygons(self):
        ...
        if self._title == "firenrt":
            ...
            logging.error(f"TODO: ERR SatelliteDetection: no setting method for the _title: {self._title}")
            sys.exit()
        
        if not perm_results["numberMatched"] == perm_results["numberReturned"]:
            logging.warning('SatelliteDetection: provided limit cuts out items of possible interest; consider upping limit') 
